...
class HoaDonNhapController extends Controller
{
    ...
    public function getThemId($id){
        $product = Products::find($id);

        return view('admin/giaodich/hoadonnhap/them',['product'=>$product]);
    }

    public function postThemId(Request $request,$id){
        ...
        if ($request->has('import_price') || $request->has('quantity')){
            ...
            if ($import_price < 0 || $quantity < 0){
                return redirect('admin/giaodich/hoadonnhap/themId/'.$id)->with('Loi','Giá tiền nhập và số lượng không thể nhỏ hơn 0');
            }else{
        ...
        if ($request->has('selling_price') || $request->has('promoted_price') || $request->has('import_price'))
        {
            ...
            if ($promoted_price < 0 || $selling_price < 0 || $import_price < 0){
                return redirect('admin/giaodich/hoadonnhap/themId/'.$id)->with('Loi','Giá nhập , giá bán sản phẩm và giá khuyến mại không thế nhỏ hơn 0');
            }else{
            ...
            if ($selling_price < $import_price){
                return redirect('admin/giaodich/hoadonnhap/themId/'.$id)->with('Loi','Giá bán sản phẩm không được nhỏ hơn giá nhập sản phẩm');
            }elseif ($import_price < $promoted_price || $selling_price < $promoted_price){
                return redirect('admin/giaodich/hoadonnhap/themId/'.$id)->with('Loi','Giá khuyến mại không được lớn hơn giá nhập và giá bán sản phẩm');
            }elseif ($promoted_price > ($selling_price - $import_price)){
                return redirect('admin/giaodich/hoadonnhap/themId/'.$id)->with('Loi','Giá khuyến mại không được lớn hơn số tiền lãi bán sản phẩm');
            }
        }
        $product->save();
        ...
        return redirect('admin/giaodich/hoadonnhap/themId/'.$id)->with('ThongBao',"Bạn đã thêm hoá đơn nhập cho sản phẩm thành công");
    }
